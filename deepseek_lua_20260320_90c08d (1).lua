...
local function createESP(player)
    ...
    local connection = RunService.RenderStepped:Connect(function()
        ...
        -- คำนวณขนาดกล่องตามระยะ
        local scale = 200 / math.max(pos.Z, 1)
        local boxWidth = math.clamp(40 * scale, 30, 100)
        local boxHeight = math.clamp(60 * scale, 45, 150)
        
        local centerX = pos.X
        local centerY = pos.Y - boxHeight/2
        ...
        drawings.distance.Position = Vector2.new(centerX, centerY + boxHeight + 5)
        ...
        drawings.health.Position = Vector2.new(centerX, centerY - 5)
...
local ESPToggle = ESPTab:Toggle({
    Title = "ESP Players",
    Desc = "แสดงกล่อง ชื่อ ระยะทาง สุขภาพ ผู้เล่น",
    Default = false,
    Callback = function(state)
        toggleESP(state)
        game:GetService("StarterGui"):SetCore("SendNotification", {
...
ESPTab:Button({
    Title = "ESP Info",
    Desc = "แสดง: กล่องครอบตัว | ชื่อ | ระยะทาง | สุขภาพ",
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "PIG HUB",
    Text = "โหลดสำเร็จ (มี Silent Aim สีรุ้ง)",
    Duration = 3
})