...
-- ========== ระบบวิ่งเร็ว (แก้ไขใหม่ ป้องกันการดึงกลับ) ==========
local walkSpeedEnabled = false
local speedMultiplier = 2
local walkConnection = nil
local walkQueue = Vector3.new(0,0,0)
local STEP_SPEED = 0.3 -- เคลื่อนที่ครั้งละ 0.3 หน่วย (ช้าลง)
...
local function setupWalkSpeed(char)
    ...
    walkConnection = RunService.Heartbeat:Connect(function()
        ...
        if moveDir.Magnitude > 0.01 then
            -- เคลื่อนที่ทีละน้อยๆ ป้องกันการเทเลพอร์ต
            local targetMove = moveDir * (speedMultiplier * 0.15)
            walkQueue = walkQueue + targetMove
            
            -- เคลื่อนที่ครั้งละ STEP_SPEED หน่วย
...
-- ========== ระบบกันตาย (แก้ไขใหม่ ไม่ดึงกลับ) ==========
...
local function enterSafeMode()
    ...
    antiDeathConnection = RunService.Heartbeat:Connect(function()
        ...
        -- ถ้าถูกดึงกลับ ให้วาร์ปกลับทันที
...
local SpeedSlider = PlayerTab:Slider({
    ...
    Desc = "ปรับความเร็วในการวิ่ง (1-5x)",
    Step = 0.5,
    Value = {
        Min = 1,
        Max = 5,
...
local ESPBoxToggle = ESPTab:Toggle({
    Title = "ESP Box",
    Desc = "แสดงกล่องครอบผู้เล่น (สีรุ้ง) - ขนาดคงที่",
    Default = false,
    Callback = function(state)
        ShowBoxESP = state
    end
...
ESPTab:Button({
    Title = "ESP Info",
    Desc = "Box (สีรุ้ง ขนาดคงที่) | Name | Health | Distance | Item Weapon | Ground Item",
    Callback = function() end
})
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "PIG HUB",
    Text = "โหลดสำเร็จ (แก้ไขวิ่งดึงกลับ + ESP Box คงที่)",
    Duration = 3
})