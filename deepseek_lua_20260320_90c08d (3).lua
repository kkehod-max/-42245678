...
local function createESP(player)
    ...
    local connection = RunService.RenderStepped:Connect(function()
        ...
        local centerX = pos.X
        local centerY = pos.Y - boxHeight/2 - 20
        
        -- อัปเดตกล่อง (เปิดตาม espBoxEnabled)
        ...
        -- ระยะทาง (เปิดตาม espDistanceEnabled)
        ...
        -- สุขภาพ (เปิดตาม espHealthEnabled)
...
-- ========== ESP TAB (แยกส่วน) ==========
ESPTab:Section({Title = "MAIN CONTROLS"})

local ESPMainToggle = ESPTab:Toggle({
    Title = "Enable ESP",
    Desc = "เปิด/ปิดระบบ ESP ทั้งหมด",
    Default = false,
    Callback = function(state)
        toggleESP(state)
        game:GetService("StarterGui"):SetCore("SendNotification", {
...
ESPTab:Divider()
ESPTab:Section({Title = "ESP COMPONENTS"})

local ESPBoxToggle = ESPTab:Toggle({
    Title = "ESP Box",
    Desc = "แสดงกรอบ 4 เส้นสีรุ้ง",
    Default = false,
    Callback = function(state)
        espBoxEnabled = state
    end
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "PIG HUB",
    Text = "โหลดสำเร็จ (ESP แยกส่วนได้ + Silent Aim)",
    Duration = 3
})