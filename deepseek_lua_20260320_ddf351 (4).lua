...
-- ========== ระบบ Bypass กันแบน + ป้องกันดึงกลับ ==========
local function setupBypass()
    ...
    -- Bypass BuyPrompt (ป้องกัน UI ค้าง)
    task.spawn(function()
...
-- ========== ระบบ ANTI-LOOK (แบบ Decoy) ==========
getgenv().AntiLookEnabled = false
getgenv().AntiLookStrength = 2
getgenv().AntiLookSpeed = 2
    ...
    for i, decoy in ipairs(AntiLookParts) do
        ...
        if originalPart and originalPart:IsA("BasePart") then
            ...
            local offset = Vector3.new(
                ...
                math.sin(time * 2 + i) * (strength * 0.5),
...
-- ========== ระบบ SILENT AIM (แบบ god2.txt + FOV 8 เหลี่ยมสีรุ้ง) ==========
...
local WalkSpeedToggle = PlayerTab:Toggle({
    Title = "Walk Speed",
    Desc = "เปิดใช้งานวิ่งเร็ว (แบบ god2.txt)",
    Default = false,
...
PlayerTab:Section({Title = "🛡️ ANTI LOOK (Decoy)"})

local AntiLookToggle = PlayerTab:Toggle({
    Title = "Anti Look",
    Desc = "เปิด: คนอื่นเห็นเราสั่น (Decoy)",
    Default = false,
    Callback = function(state)
...
ESPTab:Button({
    Title = "ESP Info",
    Desc = "Box (45) | Name | Health | Distance",
    Callback = function() end
})

-- ========== PVP TAB ==========
PVPTab:Section({Title = "🎯 SILENT AIM (แบบ god2.txt)"})
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "PIG HUB",
    Text = "โหลดสำเร็จ (Silent Aim god2 + FOV สีรุ้ง + Bypass)",
    Duration = 3
})