...
-- ========== ระบบพื้นฐาน (แก้ไขบัก) ==========
local Character = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
local Humanoid = Character:WaitForChild("Humanoid")
local HumanoidRootPart = Character:WaitForChild("HumanoidRootPart")

local walkSpeedEnabled = false
local speedValue = 0.03 -- ลดค่าลงเพื่อความเสถียร (จาก 0.05)
local moveConnection = nil
local lastPosition = Vector3.new()
local teleportThreshold = 30 -- ระยะที่ถือว่าเทเลพอร์ต
...
local function setupCharacter(char)
    ...
    moveConnection = RunService.RenderStepped:Connect(function()
        if walkSpeedEnabled and Humanoid and HumanoidRootPart and Humanoid.Health > 0 then
            if Humanoid.MoveDirection.Magnitude > 0 then
                local currentPos = HumanoidRootPart.Position
                local distMoved = (currentPos - lastPosition).Magnitude
                
                -- ถ้าระยะเคลื่อนที่ปกติ (ไม่เทเลพอร์ต)
                if distMoved < teleportThreshold then
                    local newPos = HumanoidRootPart.Position + (Humanoid.MoveDirection.Unit * speedValue)
...
local UNDERWORLD_OFFSET = 10 -- ลดความลึก
local FLY_SPEED = 100 -- ลดความเร็ว
local FLY_RADIUS = 15 -- ลดรัศมี
local antiDeathTimer = nil
...
local function teleportToSafeZone()
    ...
    -- ไม่ใช้ Anchored เพื่อป้องกันการดีเลย์
    hrp.Anchored = false
    ...
    antiDeathConnection = RunService.Heartbeat:Connect(function()
        ...
        -- ถ้าถูกดึงกลับ ให้วาร์ปกลับอย่างรวดเร็ว
        if currentHRP.Position.Y > targetY + 8 then
        ...
        local flyPos = Vector3.new(
            ...
            targetY + math.sin(time * 2) * 3,
            antiDeathPosition.Z + circleZ
        )
        
        currentHRP.CFrame = CFrame.new(flyPos)
        currentHRP.Velocity = Vector3.new(0, 0, 0) -- หยุดความเร็ว
    end)
    
    -- ตั้งเวลาปลดล็อค (กันค้าง)
    if antiDeathTimer then
        antiDeathTimer:Cancel()
    end
    antiDeathTimer = task.delay(10, function()
        if antiDeathActive and antiDeathEnabled then
            local char = LocalPlayer.Character
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    hrp:GetPropertyChangedSignal("Position"):Connect(function()
        if antiDeathActive then
            ...
            if hrp.Position.Y > targetY + 8 then
...
-- ========== ระบบ SILENT AIM (เหมือนเดิมทุกประการ) ==========
...
local BOX_SIZE = 70 -- ขนาดคงที่ (เพิ่มจาก 60 เป็น 70)
...
-- ฟังก์ชันรวม Render ESP (ใช้ BOX_SIZE คงที่)
...
local SpeedSlider = PlayerTab:Slider({
    ...
    Callback = function(value)
        speedValue = value * 0.03 -- ปรับค่าตาม multiplier
    end
})
...
PlayerTab:Section({Title = "🛡️ ANTI LOOK"})
...
ESPTab:Button({
    Title = "ESP Info",
    Desc = "Box (สีรุ้ง) | Name | Health | Distance | Item Weapon | Ground Item",
    Callback = function() end
})
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "PIG HUB",
    Text = "โหลดสำเร็จ (แก้ไขบัก + ESP Box ใหญ่ขึ้น)",
    Duration = 3
})