...
CampName.TextColor3 = Color3.fromRGB(255, 105, 180) -- สีชมพู
...
Gradient.Color = ColorSequence.new({
    ...
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 105, 180)), -- ชมพูกลาง
    ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 182, 193)) -- ชมพูอ่อน
})
...
local Window = WindUI:CreateWindow({
    ...
    Transparent = true,
    Theme = "Light", -- ธีมสีขาว
    Resizable = true,
    SideBarWidth = 200,
    HideSearchBar = true,
    ScrollBarEnabled = false,
    AccentColor = Color3.fromRGB(255, 105, 180), -- สีชมพู
    User = {
        Enabled = true,
...
-- ปรับสี Sidebar ให้เป็นชมพู
local function applyPinkTheme()
    pcall(function()
        if Window and Window.SideBar then
            Window.SideBar.BackgroundColor3 = Color3.fromRGB(255, 240, 245) -- ชมพูอ่อนมาก
        end
    end)
end
...
local function formatMoneyWithRainbow(amount)
    ...
    -- ทำให้ตัวเลขเป็นสีรุ้งโดยใช้ HTML color tags
    local rainbowText = ""
    for i = 1, #text do
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "PIG HUB",
    Text = "โหลดสำเร็จ (ธีมชมพู + ตัวเลขสีรุ้ง + เควส)",
    Duration = 3
})